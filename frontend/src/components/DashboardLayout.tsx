...
import { mapItems, LayerKey } from "@/data/map-data";
...
export function DashboardLayout() {
  ...
  useEffect(() => {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:5178";
    fetch(`${apiUrl}/api/players`)
      .then((response) => {
        if (!response.ok) throw new Error(`Player service returned ${response.status}`);
        return response.json();
      })
      .then((apiPlayers: Array<{ id: string; name: string; location: string; longitude: number; latitude: number; monogram: string; position: string; headshot: string | null; isEstimatedLocation: boolean }>) => {
        const playerItems = apiPlayers.map((player) => ({
          ...
          accent: "#f5eedb",
          stats: [["Position", player.position], ["Source", "THFCDB archive"], ["Club", "Tottenham Hotspur"], ["Record", "All-time archive"]] as Array<[string, string]>,
  ...
  return (
    <div className="dashboard">
      ...
      <div className="dashboard-grid">
        ...
        {view === "map" ? <MapContainer items={filteredItems} activeLayers={activeLayers} selectedId={selected.id} onSelect={(item) => setSelectedId(item.id)} /> : (
